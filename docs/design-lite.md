...
## 9. ClaudeLLMProvider — 実運用設定

| 項目 | MVP 既定 | 出典 / 切替 |
|---|---|---|
| model | `claude-opus-4-7` | `.env::CLAUDE_MODEL` で上書き可。要約・スコアリング・ドラフト生成すべてに使用 |
| temperature | 0.3 (要約系) / 0.5 (draft 生成) / 0.6 (video 用 concept) | `complete()` 呼び出し側で個別指定 |
| max_tokens | 180 (短コメント) / 300 (要約) / 800 (draft) | 用途別に指定 |
| timeout | 30 秒 (`anthropic.Anthropic(timeout=30)` 既定) | `check_claude_llm.py` で 30 秒 |
| retry policy | anthropic SDK の組み込み (連続失敗時) | `complete()` 内の try/except で 1 回だけ fallback |
| fallback policy | **run_daily**: key 欠 or SDK 不在 or API 失敗 → mock fallback + manifest 記録 (silent NG) **check_claude_llm**: fallback 禁止、明示的に失敗 (exit code != 0) |
| 実運用モデル | `claude-opus-4-7` (2026-05 時点)。最新は Anthropic models docs で再確認 |
| mock fallback を許可する場面 | 自動運用の日次 pipeline (key 切れ・SDK 不在で停止させたくない) |
| mock fallback を許可しない場面 | (a) `check_claude_llm.py` (b) 本番品質確認時 (将来オプション `--no-fallback` 追加候補) |

### 失敗時の挙動マトリクス

| Situation | run_daily.py | check_claude_llm.py |
|---|---|---|
| `ANTHROPIC_API_KEY` 未設定 | warning + mock fallback、`fallback_used=["llm:claude->mock"]` 記録 | exit 2、stderr に [FAIL] 出力 |
| `anthropic` SDK 未 install | 同上 | exit 3 |
| API 呼び出しが例外 | 1 度だけ mock で再試行、`fallback_used` 記録 | exit 4、例外 type + 200 char preview |
| 応答が空 | mock fallback (内容無しと判定) | exit 5 |
| 成功 | manifest に warnings/fallback_used なし | exit 0 |
